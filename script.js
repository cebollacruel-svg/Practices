...
const DIAGNOSIS = [
  ...
  { sentence:"The beekeeper lifted the lid slowly, a golden cloud of bees rose into the warm air.", answer:"comma_splice", why:"Two whole sentences fastened by a lone comma. Cover each side — both halves stand alone. A comma is far too frail to hold them. That's a <b>comma splice</b>." },
  ...
  { sentence:"The recipe looked simple, nevertheless, the stubborn dough refused to rise.", answer:"comma_splice", why:"<b>Nevertheless</b> is a conjunctive adverb, not a coordinating conjunction. It needs a semicolon before it — this is a <b>comma splice in a fancy coat</b>." },
...
const CHOICES = [
  ...
  { broken:"The librarian whispered a warning the students ignored it completely.", prompt:"This is a run-on. Which rewrite forges it whole?",
    options:[
      {text:"The librarian whispered a warning, the students ignored it completely.", correct:false, note:"A comma alone between two sentences is a comma splice — a different error."},
      ...
      {text:"The librarian whispered a warning. Ignored completely by the students.", correct:false, note:"<i>Ignored completely…</i> is now a fragment — it lost its subject."},
    ]},
  { broken:"Although the desert looks lifeless at noon.", prompt:"This is a fragment. Which rewrite completes the thought correctly?",
    options:[
      {text:"Although the desert looks lifeless at noon, it teems with creatures after dark.", correct:true, note:"An independent clause is added, and the introductory dependent clause is followed by a comma. Perfect."},
...
const REPAIRS = [
  ...
  { broken:"The bridge swayed in the storm the engineers rushed to inspect it.", kind:"run-on",
    ...
    why:"Two sentences fused with nothing. Comma + <i>so</i> captures the cause and effect beautifully." },
  { broken:"The bakery sells out by dawn, regulars line up long before sunrise.", kind:"comma splice",
    ...
    why:"A comma can't carry two independent clauses. A semicolon links cause and result with elegance." },
...
const PARAGRAPHS = [
  ...
  { text:"Consider cost first: a bicycle asks only for the occasional tire, whereas a car devours fuel, insurance, and repairs. Next, speed: the car swallows long highways with ease, but the bicycle slips through gridlock the car cannot escape. Finally, health: the cyclist arrives with stronger legs, while the driver arrives exactly as sedentary as he left.", answer:"point", why:"Cost, then speed, then health — each criterion measured on BOTH machines in the same breath. <b>Point-by-point.</b>" },
  { text:"A paper book is a creature of the senses. It carries the weight of its pages, it smells faintly of dust and glue, and it wears its history in cracked spines and dog-eared corners. An e-reader lives by entirely different rules. It weighs nothing in a thousand-volume library, it glows in the dark, and it forgets every fingerprint the moment you close it.", answer:"block", why:"All of the paper book's traits are described first, then the e-reader receives its own full treatment. <b>Block blueprint.</b>" },
  { text:"My two brothers could not be more different. In temperament, Mateo is a still lake while Diego is a crashing wave. In their habits, Mateo rises before dawn, whereas Diego greets the world at noon. Even in their dreams they diverge: Mateo longs to build bridges, but Diego longs to tear down walls.", answer:"point", why:"Temperament, habits, dreams — each criterion applied to BOTH brothers at once. Criterion-by-criterion = <b>point-by-point</b>." },
];
...
const SIGNAL = {
  ...
  junctions:[
    {left:"the tide slips out at dawn",right:"it leaves a hundred glittering pools behind",why:"Two complete thoughts. The second <b>adds</b> to the first — comma + <i>and</i> flows best. A lone comma is a <b>comma splice</b>; nothing at all is a <b>run-on</b>."},
    {left:"it leaves a hundred glittering pools behind",right:"each pool looks calm on the surface",why:"A fresh idea opens here. A <b>period</b> reads cleanest. The lone comma (splice) and the missing signal (run-on) are both errors."},
    {left:"each pool looks calm on the surface",right:"it hides a fierce little world below",why:"The second thought <b>contradicts</b> the first — calm above, fierce below — so comma + <i>but</i> captures the contrast. A period is fine too."},
...
const GAMES = [
  {id:"diagnose", num:"TRIAL I", title:"The Diagnosis Lab", desc:"Inspect each specimen and name its disease: fragment, comma splice, run-on — or a healthy sentence. Beware the impostors.", tag:"Identify · 10 specimens", arenaName:"Trial I · Diagnosis", prompt:"What ails this sentence?"},
  {id:"choose",   num:"TRIAL II", title:"The Right Hammer", desc:"The flaw is named. Four rewrites tempt you — only one heals without wounding.", tag:"Choose the fix · 5 rounds", arenaName:"Trial II · The Right Hammer", prompt:"Pick the one true repair."},
  ...
  {id:"structure",num:"TRIAL V", title:"Read the Blueprint", desc:"Comparison–contrast paragraphs are built two ways. Name the architecture: point-by-point, or block?", tag:"Identify structure · 5 paragraphs", arenaName:"Trial V · The Blueprint", prompt:"Which blueprint built this paragraph?"},
];
...
/* ── STATE ───────────────────────────────────── */
let currentGame = null, order = [], pos = 0, score = 0, streak = 0;
let answeredThis = false;
let lastChoiceOrder = [], lastStructOrder = [];
let totalScore = 0;
...
function renderDiagnose(specimen, options) {
  ...
  FLAW_ORDER.forEach((flaw,i) => {
    ...
    b.addEventListener("click", () => { if(answeredThis) return; lockOptions(i,[FLAW_ORDER.indexOf(item.answer)],item.why); });
...
function renderStructure(specimen, options) {
  ...
  lastStructOrder.forEach((st,i) => {
    ...
    b.addEventListener("click", () => { if(answeredThis) return; lockOptions(i,[lastStructOrder.indexOf(item.answer)],item.why); });
    options.appendChild(b);
  });
}
...
function revealRepair(success) {
  ...
  if (success) { score++; streak++; totalScore++; } else { streak = 0; }
...
function lockOptions(chosenIdx, correctIdxs, why) {
  answeredThis = true;
  ...
  document.getElementById("verdict").hidden = false;
  // update cover score
  document.querySelector(".stat-score-display").closest(".stat").querySelector(".stat-val").textContent = totalScore;
}
