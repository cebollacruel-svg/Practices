...
function buildTokens(specs) { return specs.map(tk); }
const PUNCTUATE = [
    {
...
function renderQuestion() {
    ...
    else if (current === "punctuate")
        renderPunctuate(specimen, options);
...
function renderDiagnose(specimen, options) {
    ...
    FLAW_ORDER.forEach((flaw, i) => {
        ...
        b.addEventListener("click", () => { if (!answeredThis)
...
function renderChoose(specimen, options) {
    ...
    lastChoiceOrder.forEach((opt, i) => {
        ...
        b.addEventListener("click", () => { if (!answeredThis)
            lockOptions(i, [lastChoiceOrder.findIndex((o) => o.correct)], opt.note); });
        options.appendChild(b);
    });
}
/* ----------------------- STRUCTURE ----------------------- */
...
/* ----------------------- PUNCTUATE (Trial IV) ----------------------- */
function renderPunctuate(specimen, options) {
    ...
    marks.forEach((mk) => {
        ...
        pb.addEventListener("click", () => {
            pActive = mk.m;
            Array.from(palette.querySelectorAll(".pmark")).forEach((x) => x.classList.toggle("active", x.dataset.mark === pActive));
        });
...
function checkPunctuation() {
    ...
    if (g.allOk) {
        ...
        $("#punc-check").disabled = true;
        $("#punc-reveal").style.display = "none";
        finishPunctuation(true);
    }
    else {
        const bits = [];
        if (g.misplaced > 0)
            bits.push(g.misplaced + (g.misplaced === 1 ? " mark is misplaced or wrong" : " marks are misplaced or wrong"));
        if (g.missing > 0)
            bits.push(g.missing + (g.missing === 1 ? " needed mark is still missing" : " needed marks are still missing"));
...
function finishPunctuation(success) {
    const item = PUNCTUATE[order[pos]];
...
function renderRepair(specimen, options) {
    ...
    input.addEventListener("keydown", (e) => { const ev = e; if (ev.key === "Enter" && ev.ctrlKey)
        submit(); });
    input.focus();
}
...
/* ----------------------- NEXT / END ----------------------- */
function next() { pos++; if (pos >= order.length)
    endGame();
else
    renderQuestion(); }
function endGame() {
