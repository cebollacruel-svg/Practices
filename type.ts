...
interface RepairItem {
  broken: string;
  kind: string;          // shown to student: "comma splice" / "run-on"
  accepted: string[];    // normalised-comparable correct rewrites
  sampleFix: string;     // shown after answering
  why: string;
}
...
type GameId = "diagnose" | "choose" | "repair" | "structure";
...
const TIME_LIMITS: Record<GameId, number> = { diagnose: 25, choose: 30, repair: 60, structure: 45 };
...
function startTimer(): void {
  stopTimer();
  timeLeft = TIME_LIMITS[current as GameId];
  ...
  timerHandle = window.setInterval(() => {
    ...
    if (timeLeft <= 8) wrap.classList.add("is-low");
    if (timeLeft <= 0) { stopTimer(); onTimeout(); }
  }, 1000);
}
function stopTimer(): void { if (timerHandle !== null) { clearInterval(timerHandle); timerHandle = null; } }

function onTimeout(): void {
  if (answeredThis) return;
  if (current === "diagnose") lockChoices(-1, FLAW_ORDER.indexOf(DIAGNOSIS[order[pos]].answer), DIAGNOSIS[order[pos]].why, true);
  else if (current === "choose") { const item = CHOICES[order[pos]]; const ci = lastChoiceOrder.findIndex((o) => o.correct); lockChoices(-1, ci, item.options.find(o=>o.correct)!.note, true); }
  else if (current === "structure") { const item = PARAGRAPHS[order[pos]]; const ci = lastStructOrder.indexOf(item.answer); lockChoices(-1, ci, item.why, true); }
  else if (current === "repair") revealRepair(false, true);
}
...
function renderRepair(specimen: HTMLElement, options: HTMLElement): void {
  ...
  input.addEventListener("keydown", (e) => {
    if ((e as KeyboardEvent).key === "Enter" && (e as KeyboardEvent).ctrlKey) submit();
  });
  input.focus();
}

function revealRepair(success: boolean, timedOut: boolean): void {
  answeredThis = true;
  stopTimer();
  const item = REPAIRS[order[pos]];
  const input = $("#repair-input") as HTMLTextAreaElement;
...
function lockChoices(chosenIdx: number, correctIdx: number, why: string, timedOut: boolean): void {
  answeredThis = true;
  stopTimer();
